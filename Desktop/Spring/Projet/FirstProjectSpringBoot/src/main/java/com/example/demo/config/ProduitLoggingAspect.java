...
public class ProduitLoggingAspect {
	...
	@Before("execution(* com.example.demo.servicesImpl.ProduitServiceImpl.*(..))")
	public void logMethodEntry(JoinPoint joinPoint) {
	String name = joinPoint.getSignature().getName();
	l.info("Produit In method " + name + " : ");
	}

	@After("execution(* com.example.demo.servicesImpl.ProduitServiceImpl.*(..))")
	public void logMethodExit(JoinPoint joinPoint) {
		String name = joinPoint.getSignature().getName();
		l.info("Produit Out Of method " + name + " : ");
		}
	@Before("execution(* com.example.demo.servicesImpl.DetailProduitServiceImpl.*(..))")
	public void logMethodEntry2(JoinPoint joinPoint) {
	String name = joinPoint.getSignature().getName();
	l.info("Detail Produit In method " + name + " : ");
	}

	@After("execution(* com.example.demo.servicesImpl.DetailProduitServiceImpl.*(..))")
